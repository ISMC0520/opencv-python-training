...
def apply_bluish(frame, intensity = 0.5):
    blue, green, red = 170, 40, 75
    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2BGRA)
    frame_height, frame_width, frame_channel = frame.shape
    bluish_bgra = (blue, green, red, 1)
    overlay = np.full((frame_height, frame_width, 4), bluish_bgra, dtype = 'uint8' )
    frame = cv2.addWeighted(overlay, intensity, frame, 1.0, 0)
    frame = cv2.cvtColor(frame, cv2.COLOR_BGRA2BGR)
